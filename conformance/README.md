...
### Round forty-seven: CSS nesting, which is a desugaring and not a splitter fix
...
a feature and not a bug fix. **Landed in round forty-seven, as exactly
...
and `attr()` landed in round forty-seven; `all` and `@scope` are still
open, and round forty-seven's entry carries a full `@scope` measurement
