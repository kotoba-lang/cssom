(ns conformance.cdp-dump
  "Reads the conformance measurement block out of a Chromium browser that
   ...
   the same `<pre id=\"kotoba-conformance-out\">` shape `--dump-dom` would
  ...
  (let [deadline (+ (js/Date.now) measure-timeout-ms)]
    (letfn [(attempt []
              (-> (send "Runtime.evaluate"
                        {:expression "(function(){var e=document.getElementById('kotoba-conformance-out');return e?e.textContent:null})()"
                         :returnByValue true})
                  (.then (fn [r]
        ...
        (.then (fn [{:keys [send close!]}]
                 (-> (send "Page.enable" {})
                     ...
                     (.then (fn [block]
                              ...
                              (emit! out-file (str "<pre id=\"kotoba-conformance-out\">" block "</pre>\n"))))
