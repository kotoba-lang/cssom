...
(defn parse-rules
  "Parses raw CSS text into rule maps. Rules nested inside an `@media (...)`
   ...
   CSS NESTING is handled before any of the above, by `desugar-nesting`
   rewriting the raw text into flat rules -- so a nested rule reaches every
   splitter here as an ordinary top-level one, and nesting composes with
   `@media`/`@layer`/`@container` in both directions with no code in any of
   them. That pass is a no-op on a stylesheet with no nested rule in it.
  ...
  (let [css (desugar-nesting css)
        ...
        ;; The `@property` registry rides here as ONE selector-less
        ...
        ;; :rule/media nil means `rule-applies-to-viewport?` keeps it. It
        ;; is emitted only when there is a registration to carry.
