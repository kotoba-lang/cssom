        ...
        (let [cst (when (map? child) (node-style child theme))
              ...
              lay (fn [shift]
                    (layout-node theme (+ content-x ml) y
                                 ...
                                 opacity inherited child
                                 (when band-for-child
                                   (mapv #(-> % (update :y - shift) (assoc :intruding? true))
                                         band-for-child))))
              laid (lay 0)
              ...
              ;; second pass: see `lay` above. Only the DRAW ops and escaped
              ;; floats can differ (the band changes where lines break, not
              ;; the child's own margins), so `gap-before`/`mt*`/`mb*` stay
              ;; the ones the first pass produced.
              laid (if (and band-for-child (pos? gap-before)) (lay gap-before) laid)
